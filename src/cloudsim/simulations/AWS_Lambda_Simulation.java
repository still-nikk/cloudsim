...
public class AWS_Lambda_Simulation {
    ...
    public static void main(String[] args) {
        try {
            ...
            // VM parameters: id, userId, mips, numberOfPes, ram, bandwidth, storage, vmm,
            // cloudletScheduler
            ...
            for (CloudletExecutionInfo info : executionSchedule) {
                ...
                if (warmFunctions.containsKey(functionType)) {
                    ...
                    for (Map.Entry<Integer, Double> entry : new ArrayList<>(vmExecutionTimes.entrySet())) {
                        ...
                        if (currentTime - lastExecutionTime <= WARM_START_DURATION) {
                            ...
                            double currentMemoryUsage = calculateCurrentMemoryUsage(currentTime,
                                    vmMemoryUsage.get(candidateVmId));
                ...
                // If no warm VM found or warm VM doesn't have enough memory, find VM with
                // enough memory or create new VM
                if (!foundWarmVM) {
                    double memoryNeeded = 0.4 * 1024; // 40% of VM RAM
                    ...
                    if (!vmFound) {
                        ...
                        // Create new VM
                        Vm newVm = new Vm(nextVmId, brokerId, 1000, 2, 1024, 10000, 10000, "Xen",
    ...
    private static void printWarmStatus(double currentTime, Map<Integer, Map<Integer, Double>> warmFunctions,
            List<Vm> vmList) {
    ...
    private static void printMemoryUsage(double currentTime, Map<Integer, List<MemoryUsageRecord>> vmMemoryUsage,
            List<Vm> vmList) {
        System.out.println("\n================ VM MEMORY USAGE ================");
        System.out.println("Current time: " + currentTime);
        System.out.println("+------+-------------------+------------------+");
    ...
    private static Datacenter createDatacenter(String name) {
        ...
        hostList.add(new Host(0, new RamProvisionerSimple(8192), new BwProvisionerSimple(50000), 1000000, peList,
                new VmSchedulerTimeShared(peList)));

        try {
            return new Datacenter(name,
                    new DatacenterCharacteristics("x86", "Linux", "Xen", hostList, 10, 3.0, 0.05, 0.1, 0.001),
    ...
    /**
     * Utility class to represent a CloudSim UtilizationModel that always returns
     * 100% utilization
     */
    ...
    static class MemoryUsageRecord {
        ...
        private boolean isStart; // true if this is when memory starts being used, false if it's when memory is
                                 // freed
